 ...
 #   def _get_out_directory(self, trainer: pl.Trainer) -> str:
  #      if trainer.logger is not None:
   ...
   #         version, name = trainer.training_type_plugin.broadcast((version, trainer.logger.name))
    ...
    def _get_out_directory(self, trainer: pl.Trainer) -> str:
        if trainer.logger is not None:
            save_dir = getattr(trainer.logger, 'save_dir', None) or trainer.default_root_dir
            ...
            version, name = trainer.strategy.broadcast((version, trainer.logger.name))
            data_path = os.path.join(save_dir, str(name), version)
        else:
            data_path = trainer.default_root_dir
        return data_path
